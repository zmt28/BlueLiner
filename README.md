...
- **National coverage (lower 48)** -- all 50 states + DC via the state selector, with USGS gauges, NHDPlusV2 flowlines (2.69M rows), and clickable streams (760K stream-order 3+) live for the entire lower 48. Wild/stocked trout classification on that network spans the East Coast trout belt -- Mid-Atlantic (MD/VA/PA), Northeast (NJ/VT/MA/NY), and southern Appalachians (WV/NC/GA); the live designated-water toggle (VA/MD), stocking, and hatch overlays remain narrower and expanding.
...
- **State fisheries ArcGIS REST services** -- live designated trout water for the `/api/trout` overlay (VA DWR, MD DNR); the clickable-streams data build additionally pulls PA (PASDA), NJ, VT, MA, NY (DEC), NC (NCWRC), GA (DNR), and WV (DNR) for wild/stocked stream tagging
...
- **SQLite / Postgres** -- user-content datastore (accounts, sessions, pins, catch log) **plus** NHDPlus VAA + clickable streams + public lands; SQLite locally, Postgres in production via the same `db.py`. Postgres uses a GiST `box` index for state-scale viewport queries (sub-100 ms on 760 K rows; `init_db()` migrates idempotently)
...
| `DATA_BASE_URL` | _(unset)_ | R2 / CDN base URL for `vaa.csv.gz` + `clickable_streams.geojson.gz` + `public_lands.geojson.gz` (versioned prefix, e.g. `https://data.blueliner.app/v4`). Unset ⇒ uses bundled mid-Atlantic files. |
